...
<?php
  ...
  if( $formdata ) {
    ...
    if( !$checkerrors ) {
		foreach($_SESSION['sacola'][$app['id']] as $produto) {
			$idProduto = $produto['id'];
			$quantidade = $produto['quantidade'];
			$estoqueAtual = data_info( "produtos",$idProduto,"estoque" );
			if (isset($produto['variacoes']) and !empty($produto['variacoes'])) {
				$variacoes = data_info( "produtos",$idProduto,"variacao" );
				$variacoes = json_decode($variacoes, true);
				foreach ($produto['variacoes'] as $i => $produtoVariacao) {
					$variacoesSacola = explode(",", $produtoVariacao);				
					foreach ($variacoesSacola as $variacao) {
						$estoqueVariacao = base64_encode(base64_decode($variacoes[$i]['item'][$variacao]['estoque']) - 1);
						$variacoes[$i]['item'][$variacao]['estoque'] = $estoqueVariacao;
					}
				}
				$variacoes = json_encode($variacoes);
				$updatedquery = "UPDATE produtos SET variacao = '$variacoes' WHERE id = $idProduto";
				mysqli_query($db_con, $updatedquery);			
			}
			
			$estoqueAtual = $estoqueAtual - $quantidade;
			$updatedquery = "UPDATE produtos SET estoque = $estoqueAtual WHERE id = $idProduto";
			mysqli_query($db_con, $updatedquery);			
					...
					<div class="col-md-12">
						...
						<div class="bread-box">
							...
							<div class="bread">
								...
								<a href="<?php echo $app['url']; ?>/pedido.php">Pedido Entrega</a>
					...
					<form id="the_form" method="POST">
						...
						<div class="row">
							...
							<div class="col-md-8 muda-checkout">
                               ...
                               <!-- <div class="accordion" id="preencheComLocalizacao">
                                  <div class="card">
                                    <div class="card-header" id="headingThree">
                                      <h2 class="mb-0">
                                        <button class="btn collapsed botao-acao" onclick="getLocation()" type="button" data-toggle="collapse" data-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree">
                                          Usar localização
                                        </button>
                                      </h2>
                                    </div>
                                    <div id="collapseThree" class="collapse" aria-labelledby="headingThree" data-parent="#accordionExample">
                                      <div class="card-body">
                                        ...
                                        <iframe width="100%" height="300px" id="mapalocalizacao" src="https://maps.google.com/maps?q=brasil&t=&z=13&ie=UTF8&iwloc=&output=embed" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"></iframe>
                                        
                                        <button onclick="confirmaLocalizacao()" class="btn btn-secondary collapsed botao-acao"  style='margin-bottom:50px'type="button" data-toggle="collapse" data-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree">
...
<div class="row">
	...
	<div class="<?php if(data_info('estabelecimentos',$eid,'calcularfrete')=='1'){echo 'col-md-6 col-sm-6 col-xs-6'; }else{echo 'col-md-12';}?>">
	  ...
	  <div class="form-field-default">
		  ...
		  <label>
			  <?php if(data_info('estabelecimentos',$eid,'calcularfrete')=='1'){ echo 'Fretes';}else{ echo 'Bairro/Região:'; } ?></label>
	...
	<?php if(data_info('estabelecimentos',$eid,'calcularfrete')=='1'){?>
...
<input type="hidden" id="_latitude" value="0">
<input type="hidden" id="_longetitude" value="0">
<?php if(data_info('estabelecimentos',$eid,'calcularfrete')=='2'){?>
<div class="row">
...
<div class="row">
	...
	<div class="col-md-12">
	  ...
	  <div class="form-field-default">
		  ...
		  <label>Complemento - BLOCO E APARTAMENTO</label>
...
<div class="row">
	...
	<div class="col-md-12">
	  ...
	  <div class="form-field-default">
		  ...
		  <input type="text" name="endereco_referencia" placeholder="Complemento" value="<?php echo htmlclean( $_SESSION['checkout']['endereco_referencia'] ); ?>" required>
										  ...
										  <div class="form-field-default">
									    	...
									    	$data_content['pagamento_getnet'] == "1" ) { ?>
													  ...
													  <?php } ?>
        													  ...
        													  <?php if( $data_content['pagamento_pix_mp'] == "1" ) { ?>
        													  <option value="10">PIX</option>
        													  <?php } ?>
									...
									<div class="row">
										...
										<div class="col-md-12">
										  ...
										  <div class="form-field-default">
										      ...
										      <input type="text" style="w-100" id="chave_pix" onclick="copiarChavePIX()" hidden="true" value="<?php echo $data_content['chave_pix']; ?>" />
...
const copiarChavePIX = ()=>{
	let copiarCodigoPIX = document.getElementById("chave_pix");
	copiarCodigoPIX.select();
	copiarCodigoPIX.setSelectionRange(0, 9999999999)
	document.execCommand("copy");
	alert('Chave PIX copiada!')
}
const calculaFrete = (button) => {
    button.disabled = true;
    button.innerHTML = 'Calculando...';
    $('#enviarP').prop('disabled',true);
    $.post("<?php $app['url'] ?>/app/estabelecimento/_ajax/sacola.php",{token: token,eid:<?php echo $eid; ?>,modo: 'calcularFrete',endereco_cep:$( "input[name='endereco_cep']" ).val()},(data)=>{
        $('#input-forma-entrega').html(data);
        $('#enviarP').prop('disabled',false);
        
        button.disabled = false;
        button.innerHTML = 'Calcular novamente';
