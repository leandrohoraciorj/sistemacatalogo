...
if(isset($_POST['acao'])){
    ...
    if(in_array($status,$arrayStatus)==false){
        die("Ação inválida");
    }
    
    if(data_info('pedidos',$pedido,'rel_estabelecimentos_id') != $eid){
        die("Ação inválida");
    }

    $status = mysqli_real_escape_string( $db_con, $status);
    
    if( edit_pedido( $pedido,$status ) ) {
...
if( $status ) {
	...
	$query .= "AND (status = '1' OR status = '4' OR status = '5' OR status = '6' OR status = '8') ";
}
											...
											<div class="fake-select">
												<i class="lni lni-chevron-down"></i>
													<select name="forma_pagamento">
														<option></option>
			                                            <?php for( $x = 0; $x < count( $numeric_data['tipo_pagamento'] ); $x++ ) { ?>
			                                            <option value="<?php echo $numeric_data['tipo_pagamento'][$x]['value']; ?>" <?php if( $tipo == $numeric_data['tipo_pagamento'][$x]['value'] ) { echo 'SELECTED'; }; ?>><?php echo $numeric_data['tipo_pagamento'][$x]['name']; ?></option>
			                                            <?php } ?>
													</select>
											</div>
							              </div>
				...
				<div class="col-md-12">
					...
					<table class="listing-table fake-table clean-table table-pedidos">
						...
						<tbody>
							...
							<tr class="fullwidth">
								<td>
                                    <span class="fake-table-title ">Nº</span>
								...
								<td>
                                    ...
                                    <div class="fake-table-data">
										...
										<?php if( $data['status'] == "8" ) { ?>
											...
											<span class="badge " style="background-color: green;">Pago</span>

										<?php } ?>

                                    </div>
                                    <div class="fake-table-break"></div>
                                    <select class="form-control" onchange="changeStatus(<?php echo $data['id']; ?>,this.value)" style="width:100%">
                                        <option value="1" <?php if( $data['status'] == "1" ) { echo "SELECTED";} ?> >Pendente</option>
                                        <option value="2" <?php if( $data['status'] == "2" ) { echo "SELECTED";} ?>>Concluído</option>
                                        <option value="3" <?php if( $data['status'] == "3" ) { echo "SELECTED";} ?>>Cancelado</option>
                                        <option value="4" <?php if( $data['status'] == "4" ) { echo "SELECTED";} ?>>Aceito</option>
                                        <option value="5" <?php if( $data['status'] == "5" ) { echo "SELECTED";} ?>>Saiu Para Entrega</option>
                                        <option value="6" <?php if( $data['status'] == "6" ) { echo "SELECTED";} ?>>Disponível p/ Retirada</option>
                                        <option value="7" <?php if( $data['status'] == "7" ) { echo "SELECTED";} ?>>Reembolsar</option>
                                    </select>
								</td>
								...
								<td>
                                    <span class="fake-table-title hidden-xs hidden-sm">Forma Pag.</span>
                                    <div class="fake-table-data"><?php echo numeric_data("tipo_pagamento",$data['forma_pagamento']); ?></div>
                                    	<?php if( $data['statuspagamento'] == "approved" ) { ?>
										<span class="badge " style="background-color: green;">Pago</span>
										<?php } ?>
										
                                    	<?php if( $data['statuspagamento'] == "pending" ) { ?>
										<span class="badge " style="background-color: #e5aa0e;">Pendente</span>
										<?php } ?>										
								...
								<td>
                                    ...
                                    <div class="fake-table-data">
										<div class="form-actions pull-right">
											...
											<a class="color-yellow" href="https://wa.me/55<?php echo clean_str($data['whatsapp']); ?>" target="_blank" title="Mandar mensagem"><i class="lni lni-whatsapp"></i></a>
										</div>
                                    </div>
                                    <div class="fake-table-break"></div>
								</td>
...
function changeStatus(pedido,st){
    $.post('editar/?id='+pedido,{formdata:true,status:st})
}
